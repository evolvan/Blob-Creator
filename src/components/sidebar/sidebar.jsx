...
function Sidebar() {
    ...
    const SVG_TEMPLATE =
        ...
        ${(svgState.type === 'linear gradient') ? `<linearGradient id="linear-grad" gradientTransform="rotate(${svgState.angle} 0.5 0.5)">
            <stop offset="0%" stop-color="${svgState.gradientColorOne}" />
            <stop offset="100%" stop-color="${svgState.gradientColorTwo}" />
        </linearGradient>` : ""}
        ${(svgState.type === "radial gradient") ? `<radialGradient id="radial-grad" r="100%" cx="${svgState.cx + "%"}" cy="${svgState.cy + "%"}">
            <stop offset="0%" stop-color="${svgState.gradientColorOne}" />
            <stop offset="100%" stop-color="${svgState.gradientColorTwo}" />
        </radialGradient>` : ""}
        <path fill="${svgState.fill}" stroke="${svgState.stroke}" stroke-width="${svgState.strokeWidth}" d="${svgState.shape}" transform="translate(100 100)" />
</svg>`;
        ...
        <>
            <div className={styles["main"]}>
                ...
                <div className={styles["btn-container"]}>
                    ...
                    <button type="button" className="btn btn-success" data-bs-toggle="modal" data-bs-target="#svgCodeModal" ><TbClipboardText /> Copy</button>
                ...
                <div className="modal-dialog">
                    <div className="modal-content">
                        ...
                        <div className="modal-footer">
                            <button type="button" className={`btn ${copied ? 'btn-success' : 'btn-outline-success'}`} onClick={copyHandler}><TbClipboardText /> {copied ? 'Copied' : 'Copy to clipboard'}</button>
                        </div>
                    </div>
                </div>
            </div>
